...
class FreeRadius (Resource):
    ...
    def retornar_informacoes_pelo_serial(self, id, args):
        ...
        if dados_plano[0][1] is None or dados_plano[0][1] == "":
        ...
        return {
            ...
            "dados": {
                ...
                "senha":senha,
                "senha5":senha,
                "pk_cod_cobranca_cliente_plano":dados_plano[0][0],
                "username":login[0][0],
    ...
    def alterar_grupo_regra(self, id, args):
        ...
        comando_infos = "SELECT \
                        ...
                        nas b \
                    WHERE a.username = '{}' \
                        AND a.nasipaddress = b.nasname \
                    ORDER BY a.radacctid DESC \
                    LIMIT 1 ;".format(args['username'])
        cursor.execute(comando_infos)
        infos = cursor.fetchone()
        if infos is None:
            return {
                "status":"2",
                "msg":"não foi encontrado nenhum usuario com esse username"
            }

        comando_regra_antiga = "SELECT \
                            groupname \
                        FROM\
                            radusergroup \
                        WHERE username = '{}' ;".format(args['username'])

        cursor.execute(comando_regra_antiga)
        grupo_antigo = cursor.fetchone()
        comando_update = "UPDATE \
                    radusergroup \
                SET\
                    groupname = '{}' \
                WHERE username = '{}';".format(args['regra'], args['username'])
        cursor.execute(comando_update)
        if cursor.rowcount == 0 :
            return {
                "status":"2",
                "msg":"não foi possível alterar sua regra, verifique se você já está na regra correta!"
            }
        ...
        requests.get(url = banco[13], params=querystring)
        conexao.cursor()
        data_atual = date.today()
        comando_log = "INSERT INTO bemtevi_radusergroup_alteracoes (\
            username,\
            grupoatual,\
            grupoantigo,\
            datahora\
            ...
            VALUES\
            ('{}','{}','{}','{}') ;".format(args['username'], args['regra'],grupo_antigo[0], data_atual)
        cursor.execute(comando_log)
        conexao.commit()
        conexao.close()
